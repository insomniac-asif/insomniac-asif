...
- **[doll-bot](https://github.com/insomniac-asif/doll-bot)** — one Discord bot for many servers: AI that does real admin work, but every tool is permission-tiered (MOD/ADMIN/OWNER), destructive ones are confirm-gated, and the prompt's first rule is that it may only claim an action a tool actually confirmed. Node / discord.js, 165 tools behind the gate. Honest edge: the tiers and gates are structural; the *don't-claim-what-you-didn't-do* rule is a prompt, not a proof.
